...
public class CuteInterpreter {
	...
	private Node runFunction(FunctionNode func) {
		...
		Node c = new FunctionNode(Node.Type.NOT_QUOTED,FunctionNode.FunctionType.LAMBDA);
		
		if(rhs2 != null && func.value != FunctionNode.FunctionType.LAMBDA) 
		{
			if (rhs2.getType() == Node.Type.NOT_QUOTED
					&& (!(rhs2 instanceof IntNode) && !(rhs2 instanceof BooleanNode)) && !((ListNode)rhs2).value.equals(c))
			{
				rhs2 = runExpr(rhs2);
			}
		}
		...
		if (rhs1 != null)
		{
			...
			{	//rhs1이 비지 않았고, func의 value가 define인 경우
				...
				&& func.value!= FunctionNode.FunctionType.LAMBDA )
				{
					errorLog("type error");
					flag = 1;
				}
		...
		if (rhs2 != null && func.value != FunctionNode.FunctionType.LAMBDA) {
			if (rhs2.getType() == Node.Type.NOT_QUOTED
					...
					&& func.value!= FunctionNode.FunctionType.DEFINE)
		...
		if (flag == 0) { //에러로 검출되지 않은 경우 다음의 코드가 실행이 된다.(flag로 판별)
			...
			case LAMBDA:
				insertTable(((ListNode) rhs1).value.toString(), Ltemp);
				result = runLambda(rhs2);
				
				for (i = 0; i < li.size(); i++) {
					if (li.get(i).getTempString().equals(((ListNode) rhs1).value.toString())) {
						li.remove(i);
					}
				}
				break;
	...
	private Node runList(ListNode list) {
		...
		if(!(opCode instanceof ListNode) && opCode.next==null)
		{
			for (i = 0; i < li.size(); i++) {
				if (li.get(i).getTempString().equals(opCode.toString())) {
					opCode = li.get(i).getTempNode();
				}
			}
			opCode.setType(Node.Type.QUOTED);
			return opCode;
		}
		for (i = 0; i < li.size(); i++) {
			if (li.get(i).getTempString().equals(opCode.toString())) {
				opCode = li.get(i).getTempNode();
				opCode.setType(Node.Type.NOT_QUOTED);
			}
		}
		
		if(opCode instanceof ListNode)
			opCode = ((ListNode)opCode).value;
	...
	public Node runExpr(Node rootExpr) {
		...
		{
			if (((ListNode) rootExpr).value.next != null) {
				Ltemp = ((ListNode) rootExpr).value.next;
			}
			
			Node c = new FunctionNode(Node.Type.NOT_QUOTED,FunctionNode.FunctionType.LAMBDA);
			
			if(((ListNode) rootExpr).value.equals(c) && !(((ListNode) rootExpr).value instanceof ListNode))
			{
				rootExpr.setType(Node.Type.QUOTED);
				result = rootExpr;
			}
	...
	private Node runLambda(Node ex)
	{
		Node res = null;

		ex.setType(Node.Type.NOT_QUOTED);
