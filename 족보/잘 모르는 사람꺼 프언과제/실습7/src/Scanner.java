...
public class Scanner {
	...
	public Scanner(String source) {
		this.accept = new boolean[] { false, true, true, true, true, true, true, true,
	...
	public class Parser {
		...
		private Node parseExpr() //문법에서 expr은 id,int boolean,list만 가질수 있음.
		{
			...
			if(t != null)
			{  //id,int,boolean을 검사해서 노드를 생성.
				switch(t.type){ 
					case ID:
						node = new IdNode(Node.Type.NOT_QUOTED,t.lexme);
						break;
					case INT:
						node = new IntNode(Node.Type.NOT_QUOTED, new Integer(t.lexme));
						break;
					case TRUE:
		...
		public Node parseProgram()
		{
			...
			if(null != t)
			{
				switch(t.type){ // '와 quote는 quoted형이기 때문에 expr로 list를 구성하며 타입을 변경해줌.
					case APOSTROPHE:
	...
	public class Printer
	{
		...
		{	
			...
			{			
				...
				{
					if(node.getType()==node.type.QUOTED) // quote의 경우 '로 출력되기 떄문에 출력.
						System.out.print("'");
				...
				{
					if(node.getType()==node.type.QUOTED) //처음부터 '가 들어오는 경우가 있을떄 출력.
						System.out.print("'");
	...
	{
		BufferedReader br = new BufferedReader(new FileReader("as07.txt"));
